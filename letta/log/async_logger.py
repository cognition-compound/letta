...
class AsyncLoggingHandler(logging.Handler):
    ...
    def __init__(
        self, max_workers: int = 2, queue_size: int = 1000, batch_size: int = 10, flush_interval: float = 1.0, level: int = logging.INFO
    ):
        ...
        self.executor = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="async_log")
        ...
        # Track metrics
        self.metrics = {"entries_processed": 0, "entries_dropped": 0, "processing_errors": 0, "queue_full_events": 0}

    def _start_processing_thread(self):
        """Start the background thread for processing log entries."""
        self._processing_thread = threading.Thread(target=self._process_log_queue, name="async_log_processor", daemon=True)
    ...
    def _process_log_queue(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                should_flush = len(batch) >= self.batch_size or (batch and time.time() - last_flush >= self.flush_interval)
    ...
    def _process_log_entry(self, entry: AsyncLogEntry):
        ...
        try:
            ...
            record = logging.LogRecord(
                name=entry.logger_name, level=entry.level, pathname="", lineno=0, msg=entry.message, args=entry.args, exc_info=None
            )
...
class AsyncLogger:
    ...
    def _should_use_async(self) -> bool:
        ...
        return self.async_handler is not None and not self.async_handler._shutdown_event.is_set()
    ...
    def _log(self, level: int, message: str, *args, **kwargs):
        ...
        if self._should_use_async():
            ...
            record = self.logger.makeRecord(self.logger.name, level, "", 0, message, args, None, func=None, extra=kwargs.get("extra", {}))
...
def setup_async_logging(
    max_workers: int = 2, queue_size: int = 1000, batch_size: int = 10, flush_interval: float = 1.0, level: int = logging.INFO
) -> AsyncLoggingHandler:
    ...
    _global_async_handler = AsyncLoggingHandler(
        max_workers=max_workers, queue_size=queue_size, batch_size=batch_size, flush_interval=flush_interval, level=level
    )
...
@contextmanager
def async_logging_context(max_workers: int = 2, queue_size: int = 1000, batch_size: int = 10, flush_interval: float = 1.0):
    """Context manager for async logging."""
    handler = setup_async_logging(max_workers=max_workers, queue_size=queue_size, batch_size=batch_size, flush_interval=flush_interval)
...
def auto_setup_async_logging():
    ...
    if hasattr(settings, "async_logging_enabled") and settings.async_logging_enabled:
        ...
        setup_async_logging(max_workers=max_workers, queue_size=queue_size, batch_size=batch_size, flush_interval=flush_interval)
