...
def lazy_log_with_context(
    logger: logging.Logger, level: int, message: str, expensive_context: Optional[Callable[[], Dict[str, Any]]] = None, **static_context
) -> None:
    """
...
def performance_monitored_log(
    operation_name: str, logger: Optional[logging.Logger] = None, level: int = logging.DEBUG, threshold_ms: float = 10.0
):
    """
    ...
    def log_operation_timing(self, operation_name: str, duration_ms: float):
        ...
        if duration_ms > 50:  # 50ms threshold
            self.warning_if_enabled(
                lambda: f"Slow operation detected: {operation_name}", operation=operation_name, duration_ms=round(duration_ms, 2)
            )
