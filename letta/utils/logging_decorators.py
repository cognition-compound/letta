...
def db_operation_logger(
    operation_type: str = "database_operation", log_level: str = "INFO", include_timing: bool = True, include_result_info: bool = True
):
    """
    ...
    def decorator(func: F) -> F:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            log_context = {"correlation_id": correlation_id, "trace_id": trace_id, "operation_type": operation_type, **context}
            ...
            try:
                ...
                if trace_id:
                    _log_event(
                        f"db_operation_completed",
                        {"operation_type": operation_type, "success": True, "duration_ms": log_context.get("duration_ms")},
                    )
                ...
                log_context.update({"error_type": type(e).__name__, "error_message": str(e), "success": False})

                # Log error
                logger.error(f"Failed {operation_type}: {context.get('method', 'unknown')}", extra=log_context, exc_info=True)

                # Add OpenTelemetry event
                if trace_id:
                    _log_event(
                        f"db_operation_failed",
                        {
...
def exception_handler(log_level: str = "ERROR", reraise: bool = True, fallback_value: Any = None, handle_sql_errors: bool = True):
    """
    ...
    def decorator(func: F) -> F:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Add OpenTelemetry event
                if trace_id:
                    _log_event("exception_occurred", {"error_type": type(e).__name__, "error_message": str(e), "function": func.__name__})
...
def performance_monitor(threshold_ms: Optional[float] = None, log_all: bool = False, warn_threshold_ms: Optional[float] = None):
    """
    ...
    def decorator(func: F) -> F:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if should_log:
                    perf_context = {"duration_ms": duration_ms, "trace_id": trace_id, "performance_monitoring": True, **context}
                ...
                logger.warning(f"Performance (FAILED): {context.get('method', 'unknown')} failed after {duration_ms}ms", extra=perf_context)
